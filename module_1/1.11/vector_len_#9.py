'''
Прежде чем мы приступим к операциям сравнения векторов, реализуйте
метод определения длины вектора — len для класса Vector
Для вектора a с координатами (x1, y1) его длина определяется по
следующей формуле: (x1 ** 2 + y1 ** 2) ** 0.5.
'''
class Point:
