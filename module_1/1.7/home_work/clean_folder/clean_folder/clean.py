...
def move_archives(root_dir, file_name):
    ...
    if root_dir.joinpath(file_name.stem).exists():
        unpack_archive(file_name, root_dir.joinpath(
            f"{file_name.stem}_{str(datetime.now().microsecond)}"))
    else:
...
def move_media(root_dir, file_name):
    ...
    if root_dir.joinpath(file_name.name).exists():
        ...
        file_name.replace(root_dir.joinpath(
            f"{file_name.stem}_{str(datetime.now().microsecond)}{file_name.suffix}"))
    else:
...
def sort_dir():
    ...
    pictures = {'name': 'Изображения', 'dir_name': 'images',
                'ext': pictures_ext, 'sort': True, 'tag': 'media', 'files': []}
    movies = {'name': 'Видео', 'dir_name': 'video',
              'ext': movies_ext, 'sort': True, 'tag': 'media', 'files': []}
    documents = {'name': 'Документы', 'dir_name': 'documents',
                 'ext': documents_ext, 'sort': True, 'tag': 'media', 'files': []}
    music = {'name': 'Музыка', 'dir_name': 'audio',
             'ext': music_ext, 'sort': True, 'tag': 'media', 'files': []}
    archives = {'name': 'Архиы', 'dir_name': 'archives',
                'ext': archives_ext, 'sort': True, 'tag': 'archive', 'files': []}
    unknown = {'name': 'Другое', 'dir_name': 'other', 'ext': [],
               'sort': False, 'tag': 'other', 'files': [], 'dirs': []}
    ...
    def dir_of_category(extension): return ''.join(
        [k for k, v in cat_dirs.items() if extension in v])
    ...
    src_files_gen = get_dir_obj(
        _dir, show_all_files_dirs=typeObj.FILES, categories_list=cat_dirs.keys())
...
TABLE_SYMBOLS = ('абвгґдеєжзиіїйклмнопрстуфхцчшщюяыэАБВГҐДЕЄЖЗИІЇЙКЛМНОПРСТУФХЦЧШЩЮЯЫЭьъЬЪ',
                 (*('abvhgde'), 'ye', 'zh', *('zyi'), 'yi', *('yklmnoprstuf'), 'kh', 'ts',
                  ...
                  ('ABVHGDE'), 'YE', 'ZH', *('ZYI'),
                  'Yi', *
                  ('YKLMNOPRSTUF'), 'KH', 'TS', 'CH', 'SH', 'SHCH', 'YU', 'YA', 'Y', 'YE',
                  *('_'*4)))
