'''
Добавьте в класс Contacts атрибут count_save, по умолчанию
он должен иметь значение 0.
Реализуйте магический метод __getstate__ для класса Contacts.
При упаковке экземпляра класса метод должен увеличивать значение
атрибута count_save на единицу. Таким образом, это свойство,
счетчик повторных операций упаковок экземпляра класса.
'''
